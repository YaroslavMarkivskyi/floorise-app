...
export function TodayView({ slots, dateIso, kcalFloor, kcalTarget, dateLabel, initialStreak }: Props) {
  ...
  function openRecipe(s: SlotItem) {
    setSelectedDish({
      name: s.dishName,
      kcal: s.dishKcal,
      cookTime: s.dishCookTime,
      ingredients: s.dishIngredients,
      steps: s.dishSteps,
    })
  }
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          ...
          {initialStreak > 0 && (
            <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">
              🔥 {initialStreak}{" "}
              {initialStreak === 1 ? "день" : initialStreak < 5 ? "дні" : "днів"}
            </span>
          )}
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {/* Dish name — tap to open recipe */}
                    <button
                      onClick={() => openRecipe(s)}
                      className="mt-1 text-left text-base font-semibold text-zinc-900 leading-snug hover:underline focus:outline-none"
                    >
                      {s.dishName}
                    </button>

                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="text-sm text-zinc-500">{s.dishKcal} ккал</span>
                      <button
                        onClick={() => openRecipe(s)}
      ...
      <RecipeDrawer dish={selectedDish} onClose={() => setSelectedDish(null)} />
    </>
  )
}
